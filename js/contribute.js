...
$(function () {
  ...
  function initMap() {
    ...
    map.addListener("click", (mapsMouseEvent) => {
      ...
      gpsCoordination.value = mapsMouseEvent.latLng
        .toString()
        .replace(/\(|\)/g, "");
      // get address from coordination
    ...
    searchBox.addListener("places_changed", () => {
      ...
      places.forEach((place) => {
        ...
        gpsCoordination.value = place.geometry.location
          .toString()
          .replace(/\(|\)/g, "");
  ...
  $("#photos").on("change", function (e) {
    ...
    for (var i = 0; i < photos.length; i++) {
      photo += `<img src="${URL.createObjectURL(photos[i])}" title="${
        photos[i].name
      }">`;
    }
    photo += "</div>";
  ...
  $(document).on("click", "#typeToggleBtn", function (e) {
    e.preventDefault();
    $("#commonName").val("");
    $("#commonName").attr("readonly")
      ? $("#commonName").removeAttr("readonly")
      : $("#commonName").attr("readonly", "readonly");
    if ($(this).attr("type") == "exist") {
      $(".scientificNameArea").html(
        `<input class="form-control" id="scientificName" name="scientificName">`
      );
      $(this).text("Contribute additional information").attr("type", "new");
    } else if ($(this).attr("type") == "new") {
      $(".scientificNameArea").html(
        `<select name="scientificName" id="scientificName" class="form-control selectpicker" data-live-search="true" data-size="5" title=" ">  </select>`
      );
      $(".selectpicker").selectpicker("refresh");
      $(this)
        .text("Search then If not exist, Add as a new")
        .attr("type", "exist");
    }
  });
      ...
      success: function (res) {
        ...
        if (res.status == "200") {
          ...
          $("#alert")
            .addClass("show")
            .html(
              `<div class="alert-body text-success"> <div> <strong> Success! </strong> ${res.msg} </div> <a href="">OK</a> </div>`
            );
        } else {
          $("#alert")
            .addClass("show")
            .html(
              `<div class="alert-body text-danger"> <div> <strong> Sorry! </strong> ${res.msg} </div> <a href="">OK</a> </div>`
            );
        }
      },
      error: function (e) {
